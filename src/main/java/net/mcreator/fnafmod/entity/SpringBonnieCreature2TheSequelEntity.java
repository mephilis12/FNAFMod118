...
public class SpringBonnieCreature2TheSequelEntity extends Monster {
	public SpringBonnieCreature2TheSequelEntity(PlayMessages.SpawnEntity packet, Level world) {
		this(FnafModModEntities.SPRING_BONNIE_CREATURE_2_THE_SEQUEL.get(), world);
	}

	public SpringBonnieCreature2TheSequelEntity(EntityType<SpringBonnieCreature2TheSequelEntity> type, Level world) {
		super(type, world);
		xpReward = 0;
		setNoAi(true);
			...
			public void tick() {
				...
				if (this.operation == MoveControl.Operation.MOVE_TO && !SpringBonnieCreature2TheSequelEntity.this.getNavigation().isDone()) {
					...
					if (SpringBonnieCreature2TheSequelEntity.this.isInWater()) {
						...
						float f3 = Mth.cos(SpringBonnieCreature2TheSequelEntity.this.getXRot() * (float) (Math.PI / 180.0));
						SpringBonnieCreature2TheSequelEntity.this.setZza(f3 * f1);
						SpringBonnieCreature2TheSequelEntity.this.setYya((float) (f1 * dy));
					} else {
						SpringBonnieCreature2TheSequelEntity.this.setSpeed(f1 * 0.05F);
					}
				} else {
					SpringBonnieCreature2TheSequelEntity.this.setSpeed(0);
					SpringBonnieCreature2TheSequelEntity.this.setYya(0);
					SpringBonnieCreature2TheSequelEntity.this.setZza(0);
				}
			}
		};
	}

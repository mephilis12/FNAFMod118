...
public class CrawlingToyFoxyOnEntityTickUpdateProcedure {
	public static void execute(LevelAccessor world, double x, double y, double z, Entity entity) {
		if (entity == null)
			return;
		if (world instanceof Level _lvl0 && _lvl0.isDay()) {
			if (world instanceof ServerLevel _level) {
				Entity entityToSpawn = new DayTimeToyFoxyEntity(FnafModModEntities.DAY_TIME_TOY_FOXY.get(), _level);
		...
		if (!((world.getBlockState(new BlockPos(x, entity.getY() - 1, z))).getBlock() == FnafModModBlocks.VENT.get())) {
			...
			((Entity) world.getEntitiesOfClass(ToyFoxyEntity.class, AABB.ofSize(new Vec3(x, y, z), 2, 2, 2), e -> true).stream().sorted(new Object() {

			...
			if (entity.getPersistentData().getDouble("FNAFTimer") == 0) {
				if (entity instanceof FreddyFazbearEntity) {
					if (world instanceof ServerLevel _level) {
						Entity entityToSpawn = new DayTimeFreddyEntity(FnafModModEntities.DAY_TIME_FREDDY.get(), _level);
						entityToSpawn.moveTo((entity.getPersistentData().getDouble("spawnX")), (entity.getPersistentData().getDouble("spawnY")), (entity.getPersistentData().getDouble("spawnZ")),
								(float) entity.getPersistentData().getDouble("spawnYaw"), 0);
